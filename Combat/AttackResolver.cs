...
namespace SagesOfOzvaram.Combat
{
    ...
    public static class AttackResolver
    {
        ...
        /// unit adjacent to the attacker is hit instead. Callers should already have confirmed
        /// attacker.GetEffectiveAPCost(move, sourceWeapon) &lt;= attacker.CurrentAP (and MP cost)
        /// before calling - this clamps rather than validates, so it never goes negative.
        /// </summary>
        public static List<AttackOutcome> Resolve(BaseUnit attacker, BaseUnit primaryTarget, Move move,
            Weapon sourceWeapon, HexGrid grid, Map map, IEnumerable<BaseUnit> allUnits)
        {
            ...
            var targets = move.HitsAllAdjacent
                ? GetAdjacentTargets(attacker, grid, unitsSnapshot)
                : (primaryTarget != null ? new List<BaseUnit> { primaryTarget } : new List<BaseUnit>());
            ...
            HexGrid grid, Map map, IList<BaseUnit> allUnits)
        {
            ...
            float hitChance = move.GetHitChance(attacker, sourceWeapon, distanceTiles);
